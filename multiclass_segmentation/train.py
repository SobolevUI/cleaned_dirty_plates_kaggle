...
print(DEVICE)
def train_segmentation_model(root,BATCH_SIZE,model):
    ...
    loss = CrossEntropyLoss() # for multiclass segmentation
    ...
    df_name=root+ '/' +  model.name +'train_len_'+str(len(train_dataset))+'_valid_len_'+str(len(valid_dataset))+'_bs' + \
    ...
    for epoch in range(0, EPOCHS):
        ...
        fig.suptitle(model.name + '_: encoder={ENCODER},\nloss=DiceLoss, '
        ...
        fig.savefig(root_model + '/' + model.name + ENCODER + '_' + loss_name + '_' + str(
...
if __name__ =='__main__':
    root=r'A:\pycharm_projects\plates\data\dataset_multiclass'
    BATCH_SIZE = 6
    CLASS = ['background','clean','dirty']
    model = smp.PAN(
        encoder_name=ENCODER,  # choose encoder, e.g. 'resnet18'
        encoder_weights='imagenet',
        # encoder_weights=None,
        in_channels=3,  # model input channels (1 for gray-scale images, 3 for RGB, etc.)
        classes=3,  # model output classes (number of classes in your dataset)
        activation='softmax')
    train_segmentation_model(root=root, BATCH_SIZE=BATCH_SIZE,  model=model)